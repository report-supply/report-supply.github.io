...
/* -------- Status classification --------
   A supplier needs a minimum number of total votes before it can be
   marked HIGH REPUTATION or 100% FRAUD. Without this floor, a single
   like/dislike (1 vs 0) would satisfy the "40% higher" / "30% higher"
   ratio check on its own and produce a confident-sounding verdict from
   almost no evidence. Below the floor — and whenever neither side has
   a clear lead — the status is UNKNOWN rather than a guess. */
const MIN_VOTES_FOR_VERDICT = 5;

function classify(id) {
  const {likes,dislikes}=getVotes(id);
  const total = likes + dislikes;
  if (total < MIN_VOTES_FOR_VERDICT) return 'unknown';
...
const ICON_UNKNOWN='<svg class="vote-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="1.5"/><path d="M9.5 9.4a2.5 2.5 0 0 1 4.85.85c0 1.67-2.35 1.9-2.35 3.55" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><circle cx="12" cy="16.9" r="0.9" fill="currentColor"/></svg>';
