...
/* ================================================================
   Cloudflare Pages KV Voting
   ---------------------------------------------------------------
   The frontend uses these endpoints:
     GET  /api/votes
     POST /api/vote
   The actual KV namespace is intentionally NOT hard-coded here.
   Bind your Cloudflare KV namespace in Cloudflare as:
     Variable name: SUPPLY_KV

   IMPORTANT:
   This block intentionally does not contain any Cloudflare API token,
   account secret, or .env value.
   ================================================================ */

const CLOUDflareVoteAPI = {
  async getVotes() {
    ...
    if (!response.ok) {
      throw new Error(`Vote API GET failed: ${response.status}`);
    }

    return await response.json();
  },

  async castVote(supplier, vote) {
    ...
    if (!response.ok) {
      const error = new Error(data.error || `Vote API POST failed: ${response.status}`);
...
/*
 * Global helpers for pages that need to explicitly refresh vote data.
 * Existing page code can call:
 *   await window.loadCloudflareVotes()
 *   await window.castCloudflareVote(id, 'like')
 */
window.loadCloudflareVotes = () => CLOUDflareVoteAPI.getVotes();
window.castCloudflareVote = (id, vote) => CLOUDflareVoteAPI.castVote(id, vote);
